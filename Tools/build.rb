...
class Builder
    ...
    def downloadSource
        @worker.run("git archive wf-mock | tar -x -v -C #{@env.sourcedir}")
    end
    ...
    def buildModules
        ...
        @worker.run("xcodebuild -project OCMock.xcodeproj -target OCMockLib -sdk iphoneos9.0 OBJROOT=#{@env.objroot} SYMROOT=#{@env.symroot}")
        @worker.run("xcodebuild -project OCMock.xcodeproj -target OCMockLib -sdk iphonesimulator9.0 OBJROOT=#{@env.objroot} SYMROOT=#{@env.symroot}")
