...
{
    id suffix = nil;
	[[mock stub] hasSuffix:suffix];
	[mock hasSuffix:suffix];
}
...
{
    (void)[[[mock stub] andReturn:@"foo"] initWithCharacters:[OCMArg anyPointer] length:3];
...
- (void)testAcceptsStubbedMethodWithVoidPointerArgument
{
	...
	[[mock stub] appendBytes:bytes length:0];
	[mock appendBytes:bytes length:0];
}
...
- (void)testAcceptsStubbedMethodWithPointerPointerArgument
{
	...
	(void)[[mock stub] initWithContentsOfFile:@"foo.txt" encoding:NSASCIIStringEncoding error:&error];
	(void)[mock initWithContentsOfFile:@"foo.txt" encoding:NSASCIIStringEncoding error:&error];
}
